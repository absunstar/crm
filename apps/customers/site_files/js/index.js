app.controller("customers", function ($scope, $http, $rootScope, $timeout) {
  ...
  $scope.add_approve = false;
  $scope.checkMobileExist = function (mobiles) {
    ...
    if(mobiles){
        ...
        $http({
          method: "POST",
          url: "/api/customers/viewmobile",
          data: { mobiles }
        }).then(
          function (response) {
            $scope.busy = false;
            if (!$scope.add_approve && response.data.done == true ) {
              $scope.warning = "##word.warningmessage##";
              $scope.error = "";
              $scope.add_approve = true;
              return
            }else{
              $scope.add($scope.customer);
              $scope.add_approve = false;
