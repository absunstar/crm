...
$scope.addToItemSell = function () {
  let exist = false;
  $scope.ticket.item_sell_list = $scope.ticket.item_sell_list || [];
  $scope.ticket.item_sell_list.forEach((itm, i) => {
  ...
  if (!exist) {
    $scope.ticket.item_sell_list.push($scope.itemSell);
  }
  $scope.itemSell = {};
};


$scope.deleteItemSell = function (item) {
  for (let i = 0; i < $scope.ticket.item_sell_list.length; i++) {
    if ($scope.ticket.item_sell_list[i].name == item.name) {
      $scope.ticket.item_sell_list.splice(i, 1);
    }
  }
};
