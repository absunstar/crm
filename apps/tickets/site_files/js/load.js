  ...
  $scope.searchAll = function () {
    if($scope.serach && $scope.serach.print_status){
  ...
  $scope.loadAll = function (where, n  , limit) {
    ...
    $http({
      ...
      function (response) {
        ...
        if (response.data.done && response.data.list.length > 0) {
          ...
          $scope.ExportedList = response.data.list;
          if (n && n == 2) {
          ...
          $scope.nodate_message = "##word.nodate##"
          $scope.count = 0;
        }
      },
      function (err) {
  ...
  $scope.loadTickets_Status = function () {
    ...
    $http({
      ...
      function (response) {
        ...
        $scope.eng_tickets_status = [];
        $scope.tickets_status.forEach(s => {
          if (s.id != 1 ){
            $scope.eng_tickets_status.push(s);
          }
        });
      },
      function (err) {
  ...
  $scope.header = ['رقم البلاغ' ,  'حالة البلاغ' , 'أهمية البلاغ'  ,  'أسم العميل ' , 'تليفون العميل' , 'عنوان العميل' , 'الشكوى' , 'الجهاز' ]
    ...
    if ($scope.count && $scope.count >0){
      ...
      $http({
        ...
        function (response) {
         ...
         window.location.href="/api/export_tickets/excel/download" ;
          },
        function (err) {
          $scope.busy = false;
          $scope.error = err;
