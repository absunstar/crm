module.exports = function init(site) {
  ...
  site.onPOST("/api/amounts_out/add", (req, res) => {
    let response = {}
    response.done = false
    ...
    amounts_out_doc._created = site.security.getUserFinger({$req : req , $res : res})
  ...
  site.onPOST("/api/amounts_out/update", (req, res) => {
    let response = {}
    response.done = false
    if (req.session.user === undefined) {
      res.json(response)
  ...
  site.onPOST("/api/amounts_out/delete", (req, res) => {
    let response = {}
    response.done = false
    if (req.session.user === undefined) {
      res.json(response)
  ...
  site.onPOST("/api/amounts_out/view", (req, res) => {
    let response = {}
    response.done = false
    $amounts_out.findOne({
      where: {
  ...
  site.onPOST("/api/amounts_out/all", (req, res) => {
    let response = {}
    response.done = false
