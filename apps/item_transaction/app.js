module.exports = function init(site) {
  ...
  site.on('change item status', itm => {
    ...
    $item_transaction.findOne({sort : {id : 1} , where : { size : itm.size , 'store.id' : itm.store.id , name:itm.name , 'company.id' : itm.company.id, current_status:'debt' , 'eng.id':itm.eng.id } , limit : 1} , (err , docs)=>{
  ...
  site.on('please track item', itm => {
    ...
    if(itm){
      ...
      $item_transaction.findMany({sort : {id : -1} , where : { size : itm.size , 'store.id' : itm.store.id  , 'company.id' : itm.company.id , name:itm.name } , limit : 1} , (err , docs)=>{
  ...
  site.on('please out item', itm => {
    ...
    $item_transaction.findMany({sort : {id : -1} , where : {'store.id' : itm.store.id, 'company.id' : itm.company.id , size : itm.size , name:itm.name } , limit : 1} , (err , docs)=>{
